...
void QueueEnqueue(Queue q, Item it) {
	// Resize the queue into double when the size reached capacity.
	
	if (q->capacity == (q->frontIndex + q->size)) {
		q->items = realloc(q->items, 2 * sizeof(Item) * q->capacity);
		q->capacity *= 2;
	} 
...
Item QueueDequeue(Queue q) {
	Item dequeueItem = q->items[q->frontIndex];
