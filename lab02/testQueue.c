...
// tests really long queue
// tests really large numbers in the queue 
// tests negative number, 0, and double number in the queue
// 
static void testQueue4(void) {
	Queue q = QueueNew();
	// enqueue 100 million numbers from 1 to 100 million
	for (int i = 1; i <= 100000000; i++) {
		QueueEnqueue(q, i);
		assert(QueueSize(q) == i);
	}
	// dequeue 1 to 10 million
