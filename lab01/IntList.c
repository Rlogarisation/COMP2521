...
void IntListInsertInOrder(IntList L, int v)
{
	...
	// When the inserted link list is empty
	if (L->first == NULL) {
		L->first = L->last = newNode;
		L->size++;
		// Must remember this return,
		// Otherwise will cause infinity amount of loop
		return;
	}
	
	// When the inserted integer is larger than the 
	// number at the end of the list (Best situation)
	if (newNode->data >= L->last->data) {
		L->last->next = newNode;
		L->last = newNode;
	}
	// When the inserted integer is smaller than first
	else if (newNode->data <= L->first->data) {
		newNode->next = L->first;
		L->first = newNode;
	}
	// All other situation in the middle
	else {
		// Loop until the corresponding position.
