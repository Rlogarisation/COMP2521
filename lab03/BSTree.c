...
/*
Another Approach:
I will create an array of structs, 
using variables to track of front and rear.
The benifit of this approach,
in which the root not only contains value,
but also the tree structure, 
easier to access and move in next direction.

// initialise a queue contains structs.
	BSTNode **array = malloc(50 * sizeof(BSTNode*));
	// initial 2 pointers to track both ends of the array
	int front = 0;
	int rear = 0;
	...
	while (rootNode != NULL) {
		...
		if (rootNode->left != NULL) {
			rear++;
			array[rear] = rootNode->left;
		}
		// add its right child (if any) to the queue
		if (rootNode->right != NULL) {
			rear++;
			array[rear] = rootNode->right;
		}
		...
		array[front] = NULL;
		front++;
		// Assign the next value as rootnode.
		rootNode = array[front];
	}
...
void BSTreeLevelOrder(BSTree t)
{
	...
	// Make a quene to store value
	struct queue *q = QueueNew();
	QueueEnqueue(q,t);

	while (!QueueIsEmpty(q)) {
		struct BSTNode *temp_node = QueueFront(q);
		showBSTreeNode(temp_node);
		QueueDequeue(q);

		if (temp_node->left != NULL) {
			QueueEnqueue(q, temp_node->left);
		}

		if (temp_node->right != NULL) {
			QueueEnqueue(q, temp_node->right);
		}
...
{
	// For single side empty node,
	//   	1
	//     / 
	// 	  2  
	// This can return the function when it check right.
	if (t == NULL) {
		return 0;
	}
	// Ending condition:
	// when both left and right has no node
	if (t->left == NULL && t->right == NULL) {
		return 1;
	}
	else {
		// Preorder to check the number of leaves
